...
- **广播 counter ≠ glucoseId**:实测 counter = glucoseId + 偏移(本机 ~5370)。
  ...
  此矛盾未解,以实测为准。
- **广播有两种 packet flag(2026-07-31 晚发现)**:flag 0x01 与 0x02 携带完全相同的
  6×3 字节记录格式、同一 cipher,但**各自独立的计数序列**。设备当天 14:14 从
  flag 0x01(58xx 序列)切换为 flag 0x02(56xx 序列)——此前记录的「-254 重定基」
  其实是这次切换,不是计数器回绕。flag 语义未知。**包身份必须是 (flag, counter)**:
  两个序列数值区间会随时间重叠,若只按 counter 存库,flag 0x02 序列爬升约 5.5 小时后
  会撞上 flag 0x01 的旧行(UNIQUE 冲突)。本地存储对 flag≠0x01 的 counter 做
...
- `openanytime/storage.py:save_history_records`:写入 readings,
  ...
  **去重必须按真 glucoseId,不能按时间邻近**:广播行时间戳以捕获时刻为锚,
  ...
  flag 0x01 → 5371,flag 0x02 → 5115(由切换点取值连续性验证:flag-1 5887=4.0
  对应 id 516(Iw 4.03),flag-2 5632=4.2 对应 id 517(Iw 4.18);注意广播 est
  时间与槽位不是一回事,同 est 时刻的两行未必是同一读数)。
