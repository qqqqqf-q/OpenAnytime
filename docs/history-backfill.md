...
- **广播 counter ≠ glucoseId**:实测 counter = glucoseId + 偏移(本机 ~5370)。
  ...
  此矛盾未解,以实测为准。
- **广播的 packet flag 会不定期切换(2026-08-01 三度确认)**:flag 携带完全相同的
  6×3 字节记录格式、同一 cipher,但**各自独立的计数序列**。已观测三次切换,
  flag 单调递增:0x01 →(07-31 14:14)0x02 →(08-01 凌晨)0x03 →(08-01 下午)0x04。
  ...
  **运维教训**:广播停流先查监控日志 `unsupported packet flag`;
  新 flag 的 counter 偏移用实捕包与 0x37 拉取序列逐点锚定(取值锚定法),
  加入 `OPENANYTIME_FLAG_OFFSETS`。
