...
- **广播 counter ≠ glucoseId**:实测 counter = glucoseId + 偏移(本机 ~5370)。
  ...
  此矛盾未解,以实测为准。
- **广播的 packet flag 会不定期切换(2026-08-01 更新)**:flag 0x01/0x02/0x03
  携带完全相同的 6×3 字节记录格式、同一 cipher,但**各自独立的计数序列**。
  已观测到设备两次切换:07-31 14:14 由 0x01(58xx)切 0x02(56xx),
  08-01 凌晨约 03:00 由 0x02 切 0x03(57xx)——切换周期与语义未知。
  **flag 语义未知。包身份必须是 (flag, counter)**:
  两个序列数值区间会随时间重叠,若只按 counter 存库,新序列爬升数小时后
  会撞上旧序列的旧行(UNIQUE 冲突)。本地存储对 flag≠0x01 的 counter 做
  `+ flag*100000` 命名空间隔离(0x01 保持历史编号不变;存量 flag 0x02 行已一次性迁移)。
  **运维教训(08-01)**:解析器若写死已见过的 flag,设备一切换广播就全停
  (监控日志 `unsupported packet flag: 3`),数据只靠每小时 backfill 续命;
  广播停流先查这条日志。确认新 flag 格式相同后应立刻放行。
...
- `openanytime/storage.py:save_history_records`:写入 readings,
  ...
  **去重必须按真 glucoseId,不能按时间邻近**:广播行时间戳以捕获时刻为锚,
  ...
  在时间邻域 ±3 槽内按 (血糖, 温度) 匹配历史行后投票。本机偏移:
  flag 0x01 → 5371,flag 0x02 → 5115,flag 0x03 → 4859(前两者由切换点取值
  连续性验证:flag-1 5887=4.0 对应 id 516(Iw 4.03),flag-2 5632=4.2 对应
  id 517(Iw 4.18);flag-3 由 2026-08-01 四个实捕包与 0x37 拉取序列逐点
  锚定:counter 5794-5797 的末条记录依次对应 id 935-938;
  注意广播 est 时间与槽位不是一回事,同 est 时刻的两行未必是同一读数)。
