...
仓库不携带任何传感器私有常量。原始 Iw 管线(广播监听 + 历史回补)
只需 `OPENANYTIME_KEY`(sureClose)与 `OPENANYTIME_DEVICE_NAME` 即可跑通;
官方口径管线额外需要四样东西:
...
- **算法输入严禁跳 id**(2026-08-01 事故):官方算法的内部调用计数与
  glucoseId 强绑定,跳 id 不喂会立即进入 err=2(GLU_MG 恒 0)且整段
  会话不再自愈。构建脚本对缺洞一律前向填充后照样喂(重复值耐受),
  缺洞 id 不产出数据行,只保持状态连续;
- 采集缺洞(两通道都没有的 id)靠 backfill 事后补齐;backfill 失败时
  (设备超出范围)表格会出现时间断档,算法值不受影响;
