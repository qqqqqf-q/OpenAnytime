...
本项目是**个人互操作性研究**：对自己合法拥有的设备进行逆向，以读取本人健康数据。本仓库**不包含厂商的任何代码、固件、安装包或算法库**，所有文档均为独立观察与实验的记录。项目与设备厂商无任何关联，文中商标归其权利人所有。输出数据不得用于任何医疗决策。

使用、复制或参与本项目前，请阅读 [LEGAL.md](LEGAL.md)（法律声明与使用条件，中英双语）。代码按 [LICENSE](LICENSE)（MIT）提供。请勿提交任何厂商文件、个人健康数据、设备序列号、传感器码或密钥。
